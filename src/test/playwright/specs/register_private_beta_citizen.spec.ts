...
test.describe('register_private_beta', () => {
  ...
  test('I as an admin cannot register private beta citizen with unassignable service roles', async ({ page, setupDao }) => {
    const forbiddenPrivateBetaRoleName = `iud-role-${BuildInfoHelper.getBuildInfo(faker.word.verb())}-${faker.word.noun()}`.normalize('NFD').replace(/[\u0300-\u036f]/g, '');
    await setupDao.createRole({ name: forbiddenPrivateBetaRoleName });
    const forbiddenPrivateBetaService = await setupDao.createService({ onboardingRoleNames: [forbiddenPrivateBetaRoleName] });

    const privateBetaAdminRoleName = `iud-role-${BuildInfoHelper.getBuildInfo(faker.word.verb())}-${faker.word.noun()}`.normalize('NFD').replace(/[\u0300-\u036f]/g, '');
    await setupDao.createRole({ name: privateBetaAdminRoleName, assignableRoleNames: ['citizen'] });
    ...
    await page.selectOption('#service', forbiddenPrivateBetaService.clientId);
