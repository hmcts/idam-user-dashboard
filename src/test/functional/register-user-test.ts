...
Scenario('I as a user should be able to register new user', async ({I}) => {
...
Scenario('I as a user should be able to search for roles', async ({I}) => {
  const registerUserEmail = randomData.getRandomEmailAddress();
...
Scenario('I as a user should be shown proper error message when no role is selected', async ({I}) => {
  const registerUserEmail = randomData.getRandomEmailAddress();
  I.loginAs(DASHBOARD_USER_EMAIL, testConfig.PASSWORD);
  I.waitForText('Manage existing users');
  I.waitForText('Add new users');
